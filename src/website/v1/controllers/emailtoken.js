        ...
        const params = {
          ...
          sendEmail
            .then(async () => {
              ...
              await emailTokenModel.findOneAndUpdate({_id: emailTokenId}, {is_mail_sent: true});
            })
            .catch(async (error) => {
              console.log(error);
                // to check token was not sent or not
              await emailTokenModel.findOneAndUpdate({_id: emailTokenId}, {is_mail_sent: false});
              return error
            });
...
exports.registerEmail = async (req, res) => {
    try {
        ...
        if (userData && userData.is_email_verified) {
            return res.status(200).send(successFormat({}, 'Email', 200, 'Email already verified!'));
        }
        ...
        if(emailTokenData && (new Date() - new Date(emailTokenData.updatedAt)) / 1000 < 300) {
            return res.status(429).send(errorMsgFormat('Please try again after 5 minutes', 'register-email', 429))
        }
       ...
       let savedData = await new emailTokenModel({
        ...
        return res.status(200).send(successFormat({}, 'register-email', 200, 'Verifcation link sent to your mail.'));
    } catch (error) {
        console.log('error in register email', error.message, error.stack)
        return res.status(500).send(errorMsgFormat(error.message, 'register-email', 500))
    }
}
    ...
    try {
        ...
        jwt.verify(token, process.env.JWT_SECRET, async (err, data)=> {
            if(err) {
                ...
                return res.status(400).send(errorMsgFormat('Invalid or expired', 'email-verify', 400));
            } else {
                let emailTokenData = await emailTokenModel.findOne({
                    _id: data._id,
                ...
                await emailTokenModel.findOneAndUpdate({_id: data._id}, {is_used: true});
                // update user table
                await User.findOneAndUpdate({_id: data.user_id}, {email: data.email, is_email_verified: true});
                return res.status(200).send(successFormat({}, 'email-verify', 200, 'Email verified successfully'));
            }
        });
    } catch (error) {
        console.log('errro in verify email', error.message, error.stack)
        return res.status(500).send(errorMsgFormat(error.message, 'email-verify', 500))
    }
}